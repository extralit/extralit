...
def test_login(api_url, api_key):
    ...
    run_command(f"extralit login --api-url {api_url} --api-key {api_key}")

    # Test whoami
    run_command("extralit whoami")

    # Test info
    run_command("extralit info")
...
def test_workspace_management(client):
    ...
    run_command(f"extralit workspaces create --name {workspace_name}")

    # Test listing workspaces
    list_result = run_command("extralit workspaces list")
    ...
    # Clean up
    run_command(f"extralit workspaces delete --name {workspace_name} --force")
...
def test_file_operations(client, workspace_name):
    ...
    try:
        ...
        run_command(f"extralit files list --workspace {workspace_name}")
        ...
        try:
            ...
            run_command(
                f"extralit files upload {temp_file_path} --workspace {workspace_name} --remote-path {remote_path}"
            )

            # Test listing files (should contain the uploaded file)
            list_result = run_command(f"extralit files list --workspace {workspace_name}")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                run_command(
                    f"extralit files download {remote_path} --workspace {workspace_name} --output {output_path}"
                )
            ...
            run_command(f"extralit files delete {remote_path} --workspace {workspace_name} --force")
...
def test_document_operations(client, workspace_name):
    ...
    try:
        ...
        run_command(f"extralit documents list --workspace {workspace_name}")

        # Test adding a document with a URL
        test_url = f"https://example.com/test_{uuid.uuid4().hex[:8]}"
        run_command(f"extralit documents add --workspace {workspace_name} --url {test_url}")

        # Test listing documents (should contain the added document)
        list_result = run_command(f"extralit documents list --workspace {workspace_name}")
        ...
        run_command(f"extralit documents add --workspace {workspace_name} --pmid {test_pmid}")
        ...
        run_command(f"extralit documents add --workspace {workspace_name} --doi {test_doi}")
    ...
    try:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                run_command(f"extralit schemas upload {temp_dir} --workspace {workspace_name}")

                # Test listing schemas
                list_result = run_command(f"extralit schemas list --workspace {workspace_name}")
                ...
                with tempfile.TemporaryDirectory() as download_dir:
                    ...
                    run_command(f"extralit schemas download {download_dir} --workspace {workspace_name}")
