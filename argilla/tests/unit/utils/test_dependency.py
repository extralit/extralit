...
@pytest.mark.parametrize(
    "args, expected", [(["extralit", ["server"]], True), (["invented_package", ["invented"]], False)]
)
...
def test_is_package_with_extras_installed_raises_key_error() -> None:
    with pytest.raises(KeyError, match="'extralit' package does not provide 'invented' extra"):
        is_package_with_extras_installed("extralit", ["invented"])
