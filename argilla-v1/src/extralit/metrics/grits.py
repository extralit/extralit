...
def compute_fscore(num_true_positives, num_true, num_positives) -> Tuple[float, float, float]:
    """
...
def factored_2dmss(
    ...
    if return_substructures:
        ...
        pred_substructure = pred_cell_grid[pred_row_nums, :][:, pred_column_nums]

        return true_substructure, pred_substructure
...
def cells_to_relspan_grid(cells: List[Dict[str, List[int]]]) -> List[List[List[int]]]:
    """
...
def output_to_dilatedbbox_grid(bboxes, labels, scores):
    ...
    In the case of the itnrecal used in the PubTables-1M paper, these boxes are
...
def compute_lcs_df(array1: np.ndarray, array2: np.ndarray) -> pd.DataFrame:
    if array1.shape != array2.shape:
        raise ValueError("Input arrays must have the same shape")

    lcs_df = pd.DataFrame(index=range(array1.shape[0]), columns=range(array1.shape[1]))

    for i in range(array1.shape[0]):
        for j in range(array1.shape[1]):
            lcs_df.iat[i, j] = lcs_string(array1[i, j], array2[i, j])

    lcs_df.columns = lcs_df.iloc[0]
    lcs_df = lcs_df.iloc[1:]
