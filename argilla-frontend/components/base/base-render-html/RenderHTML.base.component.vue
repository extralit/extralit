...
export default {
  ...
  watch: {
    ...
    searchUseRegex(value) {
      const extension = this.editor.extensionManager.extensions.find(
        (extension) => extension.name == "searchAndReplace"
      );
  ...
  methods: {
    replaceAll() {
      ...
      if (
        this.editor.storage.searchAndReplace.results?.length &&
        this.searchTerm !== this.editor.storage.searchAndReplace.searchTerm
      ) {
